...
def register2():
    ...
    if len(validate)==0:
        ...
        connection.commit()
        return render_template('registration2.html',username=username)
    if len(validate)>0:
...
def register3():
    ...
    connection.commit()
    return render_template('registration3.html',username=username)
...
def runningstatus1():
    ...
    if len(x)>0 and date==Date1:
        cursor.execute("select Deptstation,Depttime,Arrivalstation,Arrivaltime from route where trainno='{}' and depttime=<'{}' and arrivaltime>='{}' ".format(Trainno,time,time))
        z=cursor.fetchall()
        ...
        z=cursor.fetchall()
        if (z[0][5]-z[0][2]<=difference and (z[0][5]-z[0][2])>0):
...
def search():
    if request.method == 'POST' and 'Startstation' in request.form and 'Endstation' in request.form:
        ...
        if Category=="General":
                ...
                o = []
                for i in z:
                        cursor.execute("SELECT Traindetails.Trainno,Trainname,Date,SLseats,SLWL,3Aseats,3AWL,2Aseats,2AWL,1Aseats,1AWL,CCseats,CCWL FROM Traindetails INNER JOIN Generalseatavailability ON Traindetails.Trainno = Generalseatavailability.Trainno WHERE Traindetails.Trainno ='{}'".format(i))
...
def b():
    ...
    return render_template("search.html",h=my_var)
...
def cancel():
        ...
        if type == "General" and category == "CC":
            cursor.execute("SELECT CCseats from Generalseatavailability")
        ...
        if type == "General" and category == "3A":
            cursor.execute("SELECT 3Aseats from Generalseatavailability")
            Nseats = cursor.fetchall()
            Nseats = Nseats[0][0] + nseats
            cursor.execute("""UPDATE Generalseatavailability SET 3Aseats = '{}' where trainno='{}' and date='{}'""".format(Nseats,Trainno,Date))
            connection.commit()
            msg="Successfully Cancelled"
        if type == "General" and category == "2A":
            cursor.execute("SELECT 2Aseats from Generalseatavailability")
            Nseats = cursor.fetchall()
            Nseats = Nseats[0][0] + nseats
            cursor.execute("""UPDATE Generalseatavailability SET 2Aseats = '{}' where trainno='{}' and date='{}'""".format(Nseats,Trainno,Date))
            connection.commit()
            msg="Successfully Cancelled"
        if type == "General" and category == "SL":
            cursor.execute("SELECT SLseats from Generalseatavailability")
            Nseats = cursor.fetchall()
            Nseats = Nseats[0][0] + nseats
            cursor.execute("""UPDATE Generalseatavailability SET SLseats = '{}' where trainno='{}' and date='{}'""".format(Nseats,Trainno,Date))
            connection.commit()
            msg="Successfully Cancelled"
        if type == "General" and category == "1A":
            cursor.execute("SELECT 1Aseats from Generalseatavailability")
            Nseats = cursor.fetchall()
            Nseats = Nseats[0][0] + nseats
            cursor.execute("""UPDATE Generalseatavailability SET 1Aseats = '{}' where trainno='{}' and date='{}'""".format(Nseats,Trainno,Date))
            connection.commit()
